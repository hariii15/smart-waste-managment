...
/**
 * Auto-assign a bin to an active route (or create one).
 * Strategy (simple):
 * - Find an active route (status=='active') and append the bin if not already present.
 * - Else create a new active route for a default driver (DRIVER_01) and set binSequence=[binID].
 *
 * In production you would:
 * - choose nearest driver, capacity, zone, etc.
 */
export async function autoAssignBinToRoute(binID) {
  if (!binID) return;

  // 1) Try to find an active route
  const activeQ = query(collection(db, 'routes'), where('status', '==', 'active'), limit(1));
  const activeSnap = await getDocs(activeQ);

  if (!activeSnap.empty) {
    const routeDoc = activeSnap.docs[0];
    const route = routeDoc.data();
    const seq = Array.isArray(route.binSequence) ? route.binSequence : [];
    if (!seq.includes(binID)) {
      await updateDoc(routeDoc.ref, {
        binSequence: [...seq, binID],
        updatedAt: new Date().toISOString(),
      });
      // mark bin as collecting
      await updateBinStatus(binID, 'collecting');
    }
    return { routeId: routeDoc.id, action: 'appended' };
  }

  // 2) No active route -> create a new one (default driver)
  const driverId = 'DRIVER_01';
  const routeData = {
    driverId,
    truckId: null,
    status: 'active',
    binSequence: [binID],
    metrics: { totalDistance: 0, estimatedTime: 0 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    autoGenerated: true,
  };
  ...
  const docRef = await addDoc(collection(db, 'routes'), routeData);
  await updateBinStatus(binID, 'collecting');
  return { routeId: docRef.id, action: 'created' };
}
